...
export function getProductForBanner({id, countryCode, requestingUrl: url} : { id: string, countryCode: string, requestingUrl: string}) {
  const cacheFn = dbCache(getProductForBannerInternal, {
    tags: [getIdTag(id, CACHE_TAGS.products), getGlobalTag(CACHE_TAGS.countries), getGlobalTag(CACHE_TAGS.countryGroups)],
  })
